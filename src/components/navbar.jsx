...
export const Navbar = ({ isDarkMode, toggleDarkMode, transactions = [], onNavigate, userProfile }) => {
  ...
  return (
    <header className="h-12 px-3 lg:h-16 lg:px-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between z-50 shrink-0 transition-colors relative">
      <div className="flex items-center gap-1.5 lg:gap-2 cursor-pointer" onClick={() => onNavigate && onNavigate('DASHBOARD')}>
        {/* CHANGED: Tiny icon on mobile to fit screen */}
        <div className="w-6 h-6 lg:w-8 lg:h-8 bg-[#ce2727] rounded-lg flex items-center justify-center shrink-0">
          <span className="material-symbols-outlined text-white text-[14px] lg:text-xl">payments</span>
        </div>
        {/* CHANGED: Tiny text on mobile */}
        <span className="text-xs lg:text-xl font-black tracking-tight text-slate-800 dark:text-white whitespace-nowrap">
      ...
      <div className="flex items-center gap-1 lg:gap-4">
        <button 
          ...
          className="p-1.5 lg:p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-500 dark:text-slate-400"
        >
          <span className="material-symbols-outlined text-[18px] lg:text-[24px]">
        ...
        <div className="relative hidden sm:block">
          <button 
            ...
            className={`p-1.5 lg:p-2 rounded-full transition-colors relative ${isInfoOpen ? 'bg-slate-100 dark:bg-slate-800 text-[#ce2727]' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400'}`}
          >
            <span className="material-symbols-outlined text-[18px] lg:text-[24px]">info</span>
          </button>
           {isInfoOpen && (
            <>
        ...
        <div className="relative group">
          <button className="p-1.5 lg:p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-500 dark:text-slate-400 relative">
            <span className="material-symbols-outlined text-[18px] lg:text-[24px]">notifications</span>
            {notifications.length > 0 && (
              <span className="absolute top-2.5 right-2.5 w-1.5 h-1.5 bg-[#ce2727] rounded-full border border-white dark:border-slate-900"></span>
            )}
          </button>
        ...
        <div className="relative">
          <div 
            ...
            className="flex items-center gap-2 lg:gap-3 pl-1 lg:pl-2 pr-1 py-1 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full cursor-pointer transition-colors border border-transparent hover:border-slate-200 dark:hover:border-slate-700 select-none"
          >
            <span className="text-sm font-semibold text-slate-700 dark:text-slate-300 hidden md:inline">
              {userProfile?.name || 'Admin User'}
            </span>
            <img 
              src={userProfile?.avatar || 'https://picsum.photos/seed/admin/100'} 
              alt="User Avatar" 
              className="w-7 h-7 lg:w-8 lg:h-8 rounded-full object-cover border-2 border-white dark:border-slate-700 shadow-sm"
            />
          </div>
